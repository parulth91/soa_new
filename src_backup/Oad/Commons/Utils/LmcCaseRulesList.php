...
class LmcCaseRulesList {
    ...
    public static function getRule1($data) {
        $points = '0';
        if ($data['ms_medical_category_description'] != 'Shape-I' && $data['whether_unfit_for_haa_cold_climate'] == true) {
            $options = ['rule_number' => '9'];
            $points = CommonLists::getPointsTable($options, $data['regimental_number'])->point;
            //$points = $points + ConstantValues::$rule9;
        }
        return $points;
    }
