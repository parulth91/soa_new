...
<!--                <td><?= $this->Number->format($playerTieSheet->id) ?></td>-->
                ...
                <td>
                    <?= $playerTieSheet->has('winner_player') ? $this->Html->link($playerTieSheet->winner_player->full_name, ['controller' => 'RegisterCandidateEventActivities', 'action' => 'view', $playerTieSheet->winner_player->id]) : '' ?>
                </td>
                <td><?= $this->Number->format($playerTieSheet->match_number) ?></td>
                <td>
                    <?= $playerTieSheet->has('player1') ? $this->Html->link($playerTieSheet->player1->full_name, ['controller' => 'RegisterCandidateEventActivities', 'action' => 'view', $playerTieSheet->player1->id]) : '' ?>
                </td>
                <td>
                    <?= $playerTieSheet->has('player2') ? $this->Html->link($playerTieSheet->player2->full_name, ['controller' => 'RegisterCandidateEventActivities', 'action' => 'view', $playerTieSheet->player2->id]) : '' ?>
                </td>
